...
fn main() {
    let input: String = std::fs::read_to_string("../../data/2023/day/7/input").expect("Cannot read input file");
...
impl Hand {
    fn parse(input: &str) -> Self {
        ...
        let mut card_counts: Vec<(i32, Card)> = cards.iter()
            ...
            .map(|(a, b)| (b, *a))
            .collect();
        card_counts.sort_by(|a, b| match a.0 == b.0 {
            true => b.1.cmp(&a.1),
            false => b.0.cmp(&a.0),
        });

        let rank: Rank = match &card_counts[..] {
